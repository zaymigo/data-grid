...
(function ($, undefined) {
    ...
    var mouseProto = $.ui.mouse.prototype, _mouseInit = $.ui.mouse.prototype._mouseInit;
    ...
    var setter = function (setter, getter) {
        return function (val) {
            if (arguments.length === 0)return getter.apply(this); else setter.apply(this, arguments)
        }
    };
    var ieTransforms = {
        "0": {
    ...
    $.widget("ui.iviewer", $.ui.mouse, {
        ...
        _create: function () {
            ...
            if (this.options.update_on_resize == true)$(window).resize(function () {
                me.update()
            });
            this.img_object = new $.ui.iviewer.ImageObject(this.options.zoom_animation);
            if (this.options.mousewheel) {
                this.container.bind("mousewheel.iviewer", function (ev,
                                                                    delta) {
                    var zoom = delta > 0 ? 1 : -1, container_offset = me.container.offset(), mouse_pos = {
                ...
                if (gesturesSupport) {
                    ...
                    this.img_object.object().bind("touchstart", function (ev) {
                        ...
                        var touches = ev.originalEvent.touches, container_offset;
                        if (touches.length == 2) {
                            container_offset = me.container.offset();
                            originalCenter = {
                                x: (touches[0].pageX +
                                touches[1].pageX) / 2 - container_offset.left,
        ...
        update: function () {
            this._updateContainerInfo();
            this.setCoords(this.img_object.x(),
                this.img_object.y())
        },
        ...
        _imageLoaded: function (src) {
            ...
            if (this.options.zoom == "fit")this.fit(true); else this.set_zoom(this.options.zoom, true);
            this._trigger("onFinishLoad",
                0, src)
        },
        fit: function (skip_animation) {
            ...
            var new_zoom = 0;
            if (choose_left)new_zoom = this.options.width / this.img_object.orig_width() * 100; else new_zoom = this.options.height / this.img_object.orig_height() * 100;
            this.set_zoom(new_zoom, skip_animation)
        },
        center: function () {
            this.setCoords(-Math.round((this.img_object.display_width() - this.options.width) /
            2), -Math.round((this.img_object.display_height() - this.options.height) / 2))
        },
        moveTo: function (x, y) {
        ...
        _correctCoords: function (x, y) {
            ...
            if (this.img_object.display_height() <= this.options.height)y = -(this.img_object.display_height() -
        ...
        set_zoom: function (new_zoom, skip_animation, zoom_center) {
            ...
            if (new_zoom < this.options.zoom_min)new_zoom = this.options.zoom_min; else if (new_zoom > this.options.zoom_max)new_zoom = this.options.zoom_max;
            if (this.current_zoom == "fit") {
                var old_x =
                    zoom_center.x + Math.round(this.img_object.orig_width() / 2);
            ...
            var new_y = util.scaleValue(util.descaleValue(old_y,
            ...
            var coords = this._correctCoords(new_x, new_y), self = this;
        ...
        find_closest_zoom_rate: function (value) {
            ...
            var mltplr =
                this.options.zoom_delta;
            var rate = 1;
            while (Math.abs(func(this.options.zoom_base, Math.pow(mltplr, rate)) - value) > Math.abs(func(this.options.zoom_base, Math.pow(mltplr, rate + 1)) - value))rate++;
            return sgn * rate
        },
        ...
        info: function (param, withoutRotation) {
            ...
            switch (param) {
                case "orig_width":
                case "orig_height":
                    if (withoutRotation)return this.img_object.angle() %
                    180 === 0 ? this.img_object[param]() : param === "orig_width" ? this.img_object.orig_height() : this.img_object.orig_width(); else return this.img_object[param]();
                case "display_width":
                case "display_height":
                case "angle":
        ...
        _mouseStart: function (e) {
            ...
            if (this._trigger("onStartDrag", 0,
                    this._getMouseCoords(e)) === false)return false;
        ...
        createui: function () {
            var me =
                this;
            $("<div>", {"class": "iviewer_zoom_in iviewer_common iviewer_button"}).bind("mousedown touchstart", function () {
                me.zoom_by(1);
                return false
            }).appendTo(this.container);
            $("<div>", {"class": "iviewer_zoom_out iviewer_common iviewer_button"}).bind("mousedown touchstart", function () {
                me.zoom_by(-1);
                return false
            }).appendTo(this.container);
            $("<div>", {"class": "iviewer_zoom_zero iviewer_common iviewer_button"}).bind("mousedown touchstart", function () {
                me.set_zoom(100);
                return false
            }).appendTo(this.container);
            $("<div>",
                {"class": "iviewer_zoom_fit iviewer_common iviewer_button"}).bind("mousedown touchstart", function () {
                    me.fit(this);
                    return false
                }).appendTo(this.container);
            this.zoom_object = $("<div>").addClass("iviewer_zoom_status iviewer_common").appendTo(this.container);
            $("<div>", {"class": "iviewer_rotate_left iviewer_common iviewer_button"}).bind("mousedown touchstart", function () {
                me.angle(-90);
                return false
            }).appendTo(this.container);
    ...
    $.ui.iviewer.ImageObject = function (do_anim) {
        this._img = $("<img>").css({position: "absolute", top: "0px", left: "0px"});
        this._loaded = false;
    ...
    (function () {
        ...
        this._dimension = function (prefix,
                                    name) {
            var horiz = "_" + prefix + "_" + name, vert = "_" + prefix + "_" + (name === "height" ? "width" : "height");
        ...
        this.display_width = this._dimension("display", "width"), this.display_height = this._dimension("display", "height"), this.display_diff = function () {
            return Math.floor(this.display_width() - this.display_height())
        };
        this.orig_width = this._dimension("orig", "width"), this.orig_height = this._dimension("orig",
            "height"), this.x = setter(function (val, skipCss) {
            this._x = val;
            if (!skipCss) {
        ...
        this.angle = setter(function (deg) {
            ...
            var cssVal = "rotate(" + deg + "deg)", img = this._img;
            ...
            if (useIeTransforms) {
                ...
                img.css({
                    marginLeft: ieTransforms[deg].marginLeft *
                    this.display_diff() / 2, marginTop: ieTransforms[deg].marginTop * this.display_diff() / 2
                })
            }
        }, function () {
            return this._angle
        ...
        this.setImageProps = function (disp_w, disp_h, x, y, skip_animation, complete) {
            ...
            if (useIeTransforms)jQuery.extend(params, {
            ...
            var swapDims = this._swapDimensions, img = this._img;
            if (useIeTransforms && swapDims) {
                var ieh =
                    this._img.width(), iew = this._img.height(), iedh = params.height - ieh;
                iedw = params.width - iew;
                delete params.width;
                delete params.height
            }
            if (this._do_anim && !skip_animation)this._img.stop(true).animate(params, {
                duration: 200,
                complete: complete,
